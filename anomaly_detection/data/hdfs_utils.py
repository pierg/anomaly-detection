...
def load_hdfs_events(
    hdfs_data_path: Path, file_subpath: str, config: dict, device: str
) -> DataLoader:
    """
    ...
    dataset = HDFSEventsDataset(
        events, window_size=config["window_size"], device=device
    )
    return DataLoader(
        dataset,
        batch_size=config["batch_size"],
        collate_fn=lambda x: custom_collate_fn(
            x, config["window_size"], config["num_features"]
        ),
    )
...
def custom_collate_fn(batch, window_size, num_features):
    ...
    batch_x_transformed = torch.stack(
        [x.view(window_size, num_features).float() for x in batch_x]
    )

    return batch_x_transformed, torch.tensor(batch_y, device=batch_x_transformed.device)


def load_and_prepare_hdfs_data(
    hdfs_data_path: Path,
    config: dict,
    ...
    train_dataset, val_dataset = random_split(
        train_val_loader.dataset, [train_size, val_size]
    )

    # Applying transformations directly in the DataLoader through a custom
    # collate function
    ...
    # Load test data
    test_normal_loader = load_hdfs_events(
        hdfs_data_path, "hdfs_test_normal", config, device
    )
    test_abnormal_loader = load_hdfs_events(
        hdfs_data_path, "hdfs_test_abnormal", config, device
    )

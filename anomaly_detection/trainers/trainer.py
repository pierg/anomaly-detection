...
class Trainer:
    ...
    def train(
        self, max_iters: int, eval_interval: int, checkpoint_interval: int = 1000
    ):
        """
        ...
        while iter_count < max_iters:
            for xb, yb in self.train_loader:
                ...
                # Assuming your model's output and targets are already shaped
                # correctly
                ...
                if iter_count % eval_interval == 0:
                    ...
                    print(
                        f"Iter {iter_count:4d} | Train Loss {train_loss:6.4f} | Val Loss {val_loss:6.4f}"
                    )
    ...
    def load_latest_checkpoint(self):
        ...
        checkpoints = [
            ckpt for ckpt in os.listdir(self.checkpoint_dir) if ckpt.endswith(".pth")
        ]
        if checkpoints:
            ...
            logger.info(
                f"Resumed from checkpoint: {latest_checkpoint} at iteration {self.start_iter}"
            )
    ...
    def evaluate(
        ...
        def process_dataset(dataset, is_normal):
            ...
            for xb, yb in dataset:
                with torch.no_grad():
                    ...
                    # Get indices of top 'num_candidates' predictions for each
                    # sample
                    top_predictions = np.argsort(scores_batch, axis=1)[
                        :, -num_candidates:
                    ]

                    # Check if true labels are among top predictions, avoiding
                    # redundant shape expansion
                    correct_predictions = np.isin(yb.cpu().numpy(), top_predictions)

                    if is_normal:
                        # For normal data, correct predictions are true
                        # positives, incorrect are false negatives
                        true_positives += correct_predictions.sum()
                        false_negatives += (~correct_predictions).sum()
                    else:
                        # For abnormal data, incorrect predictions are false
                        # positives
        ...
        logger.info(
            f"Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1_measure:.4f}"
        )
    ...
    def save_checkpoint(self, iter_count: int):
        ...
        checkpoint_path = os.path.join(
            self.checkpoint_dir, f"checkpoint_{iter_count}.pth"
        )
        torch.save(
            {

...
def plot_event_timeline(event_log, color_by="event_type"):
    ...
    if color_by not in event_log.data.columns:
        print(
            f"Column '{color_by}' not found in event log data. Defaulting to no color coding."
        )
        color_by = None
    ...
    for i, (idx, row) in enumerate(event_log.data.iterrows()):
        ax.plot(
            [idx, idx],
            [0, 1],
            color=color_vals[i] if color_by else "blue",
            marker="o",
            markersize=8,
        )
    ...
    if color_by and colors:
        patches = [
            plt.Line2D(
                [0], [0], color=color, marker="o", linestyle="", markersize=8, label=val
            )

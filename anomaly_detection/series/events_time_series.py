...
class FiniteStateMachineTimeSeries(EventSequenceTimeSeries):
    ...
    def __init__(self, data: pd.DataFrame, event_column: str, state_column: str) -> None:
        """
    ...
    def state_transitions(self) -> pd.DataFrame:
        ...
        """
        transitions = self.data.groupby([self.state_column, self.event_column]).size().unstack(fill_value=0)
        return transitions

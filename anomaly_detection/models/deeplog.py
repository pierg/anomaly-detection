...
class DeepLog(nn.Module):
    def __init__(
        ...
        logger.info(
            f"Initializing LSTM layer with input size: {input_size}, hidden size: {hidden_size}, num layers: {num_layers}"
        )
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)

        logger.info(
            f"Initializing Linear layer with input size: {hidden_size}, output size: {output_size}"
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Pass the output from the last time step through the fully connected
        # layer
        out = self.fc(out[:, -1, :])

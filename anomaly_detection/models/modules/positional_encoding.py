...
class SinusoidalPositionalEncoding(nn.Module):
    ...
    Unlike embedding-based positional encodings, sinusoidal encodings are not learned from anomaly_detection.data.
    ...
    def __init__(self, d_model, max_len=5000):
        super().__init__()
        logger.info(
            f"Initializing sinusoidal positional encoding with d_model={d_model} and max_len={max_len}"
        )
        ...
        # Add a batch dimension (at position 0) for easy addition to token
        # embeddings.
        pe = pe.unsqueeze(0)
    ...
    def forward(self, x):
        ...
        # Use broadcasting to add the positional encoding across the batch
        # dimension.
...
class EmbeddingPositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        # Determine the device (CPU/GPU) where the input tensor resides to
        # ensure compatibility.
        device = x.device
        # Extract the batch size (B), sequence length (T), and d_model from the
        ...
        # possibly with some form of embedding lookup that supports batched
        # indices.
        # Expected shape [B, T, d_model]
        pos_encodings = self.pos_embedding(position_indices)

        # Add the positional encodings to the input embeddings.
        # Assuming pos_encodings is correctly shaped as [B, T, d_model], it can
        # be directly added to x.
        x = x + pos_encodings
        return x

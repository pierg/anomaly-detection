...
class EnhancedDL(nn.Module):
    def __init__(self, 
        ...
        logger.info(f'Initializing EnhancedDL with input size: {input_size}, hidden size: {hidden_size}, '
        ...
        # Input and output sizes of the transformer block are set to hidden_size, as it processes LSTM's outputs
        self.transformer_block = Block(n_embd=hidden_size, n_head=n_heads, block_size=hidden_size, dropout=dropout)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Selecting the output from the last timestep and passing it through the linear layer
        x = self.fc(x[:, -1, :])

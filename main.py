...
from anomaly_detection.utils.paths import (checkpoints_folder,
                                           hdfs_deeplog_data_path, logs_folder)
...
def main():
    for config_name in configs:
        ...
        data_loaders = load_and_prepare_hdfs_data(
            hdfs_deeplog_data_path, config, device
        )
        train_loader, val_loader, test_normal_loader, test_abnormal_loader = (
            data_loaders
        )
        ...
        for model_name, model_variant in models:
            ...
            print(
                f"Training and evaluation completed for {config_id}. Precision: {precision}, Recall: {recall}, F1 Score: {f1_score}"
            )
